...
const (
	...
	Ext3         = "ext3"
	Ext4         = "ext4"
	ElementalCmd = "elemental --debug --logfile /tmp/elemental.log"
)
...
func (s SUT) GatherAllLogs() {
	...
	// dmesg
	_, _ = s.command("dmesg > /tmp/dmesg", true)
	s.GatherLog("/tmp/dmesg")

	// grab full journal
	_, _ = s.command("journalctl -o short-iso > /tmp/journal.log", true)
	s.GatherLog("/tmp/journal.log")

	// uname
	_, _ = s.command("uname -a > /tmp/uname.log", true)
	s.GatherLog("/tmp/uname.log")

	// disk info
	_, _ = s.command("lsblk -a >> /tmp/disks.log", true)
	_, _ = s.command("blkid >> /tmp/disks.log", true)
