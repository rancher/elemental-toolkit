...
description: >
  ...
  in AWS public cloud provider by using the Elemental Vanilla image.
---
...
Publishing an AMI image in AWS based on top of the latest Elemental Vanilla image is
...
resulting snapshot as an AMI. In such case the lastest Elemental image will be
...
* `aws_source_ami_filter_name`: This a filter to choose the AMI image for the
  build process. It defaults to `*cOS*Vanilla*` pattern to pick the latest Elemental
