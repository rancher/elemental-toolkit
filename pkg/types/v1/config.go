...
// WriteInstallState writes the state.yaml file to the given path
func (c Config) WriteInstallState(i *InstallState, path string) error {
	...
	err = c.Fs.WriteFile(path, data, constants.FilePerm)
	if err != nil {
		c.Logger.Errorf("failed to write state file '%s': %v", path, err)
		return err
	}
